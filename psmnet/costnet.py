...
from .layers import SelfAttntion, Squeeze_excitation_layer
...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = StackedBlocks(n_blocks=3, in_channels=32, out_channels=64, kernel_size=3, stride=2, padding=1, dilation=1)  # downsample
        self.attn1 = SelfAttntion(64)
...
class StackedBlocks(nn.Module):
    ...
    def __init__(self, n_blocks, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1):
        ...
        self.scse = Squeeze_excitation_layer(out_channels)
    def forward(self, inputs):
        out = self.net(inputs)
